...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    // One entry, not two: both opened the same window, so a second icon beside the first
    // only asked the user to guess which half of it they wanted. Statistics is a section
    // inside that window and stays one click away in its rail.
    ...
    diagnosticsButton->setToolButtonStyle(Qt::ToolButtonIconOnly);
    ...
    const auto retintDiagnostics = [openDiagnosticsAction, openStatsAction, diagnosticsButton] {
        ...
        // Not the action's own magnifier: this corner already holds the profile search,
        // and two glasses side by side say the second one searches something else. A pulse
        // covers what the window is for — how the connection is doing and how much ran
        // through it — where either half's icon would name only its own section.
        diagnosticsButton->setIcon(MaterialIcon::icon(MaterialIcon::Glyph::Pulse, tint, 19));
    };
    retintDiagnostics();
