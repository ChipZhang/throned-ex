...
void EditMasque::onStart(std::shared_ptr<Configs::Profile> _ent) {
    ...
    if (outbound->http_version == 2)
        ui->http_version->setCurrentIndex(kHttp2);
    else if (outbound->disable_version_fallback)
        ui->http_version->setCurrentIndex(kHttp3Only);
    else
        ui->http_version->setCurrentIndex(kHttp3Fallback);
}
