...
bool ConnectionsTreeFilterProxyModel::leafMatches(const ConnectionsTree::ProcessGroupItem *group,
    ...
    return m_target.isEmpty() || leaf->destText.contains(m_target, Qt::CaseInsensitive) || ConnectionsTreeModel::displayProcessName(group->processName).contains(m_target, Qt::CaseInsensitive);
}
...
bool ConnectionsTreeFilterProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const {
    ...
    return sourceRow >= 0 && sourceRow < static_cast<int>(group->children.size()) && leafMatches(group, group->children[sourceRow].get());
}
