...
QJsonObject xrayXHTTP::ExportToJson() {
    ...
    if (!headers.isEmpty())
        extraObj["headers"] = qStringListToJsonObject(headers);
    else
        extraObj.remove("headers");
    ...
    if (!xmuxObj.isEmpty())
        extraObj["xmux"] = xmuxObj;
    else
        extraObj.remove("xmux");
...
bool xrayStreamSetting::ParseFromLink(const QString& link) {
    ...
    if (network == "xhttp")
        ...
        ws->ParseFromLink(link);
    else if (network == "httpupgrade")
        httpupgrade->ParseFromLink(link);
    else if (network == "grpc")
        grpc->ParseFromLink(link);
...
bool xrayStreamSetting::ParseFromJson(const QJsonObject& object) {
    ...
    else if (object.contains("network"))
        network = object.value("network").toString();
    if (network == "tcp") network = "raw";
    if (!Configs::XrayNetworks.contains(network)) return false;
    if (network == "raw") {
        if (object["rawSettings"].isObject())
            rawSettings = object["rawSettings"].toObject();
        else if (object["tcpSettings"].isObject())
            rawSettings = object["tcpSettings"].toObject();
    }
    if (object.contains("security")) security = object.value("security").toString();
    if (security == "tls" && object["tlsSettings"].isObject())
        TLS->ParseFromJson(object["tlsSettings"].toObject());
    else if (security == "reality" && object["realitySettings"].isObject())
        reality->ParseFromJson(object["realitySettings"].toObject());
...
QJsonObject xrayStreamSetting::ExportToJson() {
    ...
    if (security == "tls")
        ...
        object["realitySettings"] = reality->ExportToJson();
