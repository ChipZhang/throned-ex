...
bool useXrayVless(const QString& link) {
    ...
    const bool rawHttpOverTls = (transport.isEmpty() || transport == "tcp" || transport == "raw") && query.queryItemValue("headerType") == "http" && ((!security.isEmpty() && security != "none") || !query.queryItemValue("sni").isEmpty() || !query.queryItemValue("peer").isEmpty());

    if (dataManager->settingsRepo->xray_vless_preference == Xray::AllVLESS || rawHttpOverTls || transport == "xhttp" || query.hasQueryItem("fm") || query.hasQueryItem("finalmask") || (security == "reality" && dataManager->settingsRepo->xray_vless_preference == Xray::XhttpAndReality) || (query.queryItemValue("encryption") != "none" && query.queryItemValue("encryption") != "") || query.queryItemValue("extra") != "") return true;
    return false;
}
