...
void calculatePrerequisites(BuildContext &ctx) {
    ...
    if (!routeChain->endpointProfileIDs.isEmpty()) {
        ...
        for (int endpointID: routeChain->endpointProfileIDs) {
            ...
            for (int hopID: hopIDs) {
                ...
                if (hopEnt->outbound->IsExtraCore() || isCustomFullConfig(hopEnt) || isXrayFullConfig(hopEnt) || hopEnt->type == "chain") {
    ...
    if (settings.enable_dns_routing) {
        ...
        for (const auto &[outID, outTag]: preReqs.routing.outboundMap) {
            ...
            if (chain.process.isEmpty() && chain.sites.ruleSets.isEmpty() && !chain.sites.hasInlineConditions())
                continue;
...
void buildDNSSection(BuildContext &ctx, bool useDnsObj = true) {
    ...
    if (getOS() == Linux && settings.spmode_vpn && settings.core_box_underlying_dns.isEmpty() && settings.direct_dns.startsWith("localhost", Qt::CaseInsensitive)) {
        MW_show_log(QObject::tr(
            "Direct DNS is \"localhost\" while Tun is on: this often fails on Linux. "
        ...
        if (isTailscale) {
            ...
            rules.prepend(QJsonObject{
                {"domain", QJsonArray{
                               ...
                               "login.tailscale.com",
                               "log.tailscale.io"}},
                {"domain_suffix", QJsonArray{"tailscale.com", "tailscale.net", "tailscale.io"}},
                {"action", "route"},
    ...
    if (settings.dns_optimistic && !settings.dns_disable_cache && !settings.dns_disable_expire) {
        if (settings.dns_optimistic_timeout.isEmpty())
            dnsObj["optimistic"] = true;
        else
            dnsObj["optimistic"] = QJsonObject{
                {"enabled", true},
...
QString buildOutboundChain(BuildContext &ctx, const ChainBuildRequest &req) {
    ...
    for (const auto &ent: ents) {
        if (ent->outbound->IsXray())
            ...
            if (xrayEnts.isEmpty())
                initialSingEnts.append(ent);
            else
                tailingSingEnts.append(ent);
        }
    }
    QList<int> ports;
    // A pre-probed 0 means the caller's probe failed; re-probe rather than bake in a dead port.
...
void buildWarpInFrontOfSelector(BuildContext &ctx) {
    ...
    if (warpEnt->outbound->IsEndpoint())
        ctx.endpoints.append(warpObject);
    else
        ctx.outbounds.append(warpObject);
}
...
void buildRouteSection(BuildContext &ctx) {
    ...
    if (!route.contains("find_process") && (settings.enable_stats || routeChain->UsesProcessRules()))
