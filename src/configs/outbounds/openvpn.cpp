...
bool openvpn::ParseFromJson(const QJsonObject& object) {
    ...
    if (object.contains("tunnel_dns"))
        tunnel_dns = object["tunnel_dns"].toString();
    else if (object["block_outside_dns"].toBool())
        tunnel_dns = kTunnelDnsStrict;
    else if (object.contains("use_tunnel_dns") && !object["use_tunnel_dns"].toBool())
        tunnel_dns = kTunnelDnsNone;
    return true;
}
...
BuildResult openvpn::Build(OtpCodeSession& otpCodes) {
    ...
    if (!auth_retry.isEmpty())
        object["auth_retry"] = auth_retry;
    else if (liveChallenge)
        object["auth_retry"] = "interact";
    if (!static_key.isEmpty()) object["static_key"] = QListStr2QJsonArray(static_key);
    if (!static_key_path.isEmpty()) object["static_key_path"] = static_key_path;
    if (!key_direction.isEmpty()) object["key_direction"] = key_direction;
