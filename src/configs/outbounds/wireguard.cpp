...
bool Peer::ParseFromLink(const QString& link) {
    ...
    port = url.port(51820);
    if (query.hasQueryItem("public_key"))
        public_key = QueryValue(query, "public_key");
    else if (query.hasQueryItem("publickey"))
        public_key = QueryValue(query, "publickey");
    else if (query.hasQueryItem("peer_public_key"))
        public_key = QueryValue(query, "peer_public_key");

    if (query.hasQueryItem("pre_shared_key"))
        pre_shared_key = QueryValue(query, "pre_shared_key");
    else if (query.hasQueryItem("preshared_key"))
        pre_shared_key = QueryValue(query, "preshared_key");
    else if (query.hasQueryItem("presharedkey"))
        pre_shared_key = QueryValue(query, "presharedkey");
    else if (query.hasQueryItem("psk"))
        pre_shared_key = QueryValue(query, "psk");
    ...
    if (query.hasQueryItem("persistent_keepalive_interval"))
        persistent_keepalive = QueryValue(query, "persistent_keepalive_interval");
    else if (query.hasQueryItem("persistent_keepalive"))
        persistent_keepalive = QueryValue(query, "persistent_keepalive");
    else if (query.hasQueryItem("keepalive"))
        persistent_keepalive = QueryValue(query, "keepalive");
...
bool wireguard::ParseFromLink(const QString& link) {
    if (link.contains("[Interface]") && link.contains("[Peer]")) {
        ...
        for (const auto& line: lines) {
            ...
            if (key == "privatekey")
                ...
                address = value.replace(" ", "").split(",", Qt::SkipEmptyParts);
            else if (key == "mtu")
                mtu = value.toInt();
            else if (key == "publickey")
                peer->public_key = value;
            else if (key == "presharedkey" || key == "preshared_key" || key == "psk")
                peer->pre_shared_key = value;
            else if (key == "persistentkeepalive" || key == "persistent_keepalive")
                ...
                jc = value.toInt(), enable_amnezia = true;
            else if (key == "jmin")
                jmin = value.toInt(), enable_amnezia = true;
            else if (key == "jmax")
                jmax = value.toInt(), enable_amnezia = true;
            else if (key == "s1")
                s1 = value.toInt(), enable_amnezia = true;
            else if (key == "s2")
                s2 = value.toInt(), enable_amnezia = true;
            else if (key == "s3")
                s3 = value.toInt(), enable_amnezia = true;
            else if (key == "s4")
                s4 = value.toInt(), enable_amnezia = true;
            else if (key == "h1")
                h1 = value, enable_amnezia = true;
            else if (key == "h2")
                h2 = value, enable_amnezia = true;
            else if (key == "h3")
                h3 = value, enable_amnezia = true;
            else if (key == "h4")
                h4 = value, enable_amnezia = true;
            else if (key == "i1")
                i1 = value, enable_amnezia = true;
            else if (key == "i2")
                i2 = value, enable_amnezia = true;
            else if (key == "i3")
                i3 = value, enable_amnezia = true;
            else if (key == "i4")
                i4 = value, enable_amnezia = true;
            else if (key == "i5")
                i5 = value, enable_amnezia = true;
            else if (key == "headerprotectionkey" || key == "header_protection_key")
                header_protection_key = value, enable_amnezia = true;
            else if (key == "contentpaddingaddition" || key == "content_padding_addition")
                content_padding_addition = value, enable_amnezia = true;
            else if (key == "rekeyaftertime" || key == "rekey_after_time")
                rekey_after_time = value, enable_amnezia = true;
            else if (key == "rekeytimeout" || key == "rekey_timeout")
                rekey_timeout = value, enable_amnezia = true;
            else if (key == "rejectaftertime" || key == "reject_after_time")
                reject_after_time = value, enable_amnezia = true;
            else if (key == "keepalivetimeout" || key == "keepalive_timeout")
                keepalive_timeout = value, enable_amnezia = true;
            else if (key == "maxhandshakeattempts" || key == "max_handshake_attempts")
                max_handshake_attempts = value, enable_amnezia = true;
            else if (key == "randomtrailers" || key == "random_trailers")
                random_trailers = ParseAmneziaBool(value), enable_amnezia = true;
            else if (key == "disablecookies" || key == "disable_cookies")
                disable_cookies = ParseAmneziaBool(value), enable_amnezia = true;
        }
        FixAddress();
    ...
    if (query.hasQueryItem("private_key"))
        private_key = QueryValue(query, "private_key");
    else if (query.hasQueryItem("privatekey"))
        private_key = QueryValue(query, "privatekey");
    else if (!url.userName().isEmpty())
    ...
    if (query.hasQueryItem("header_protection_key"))
        header_protection_key = QueryValue(query, "header_protection_key"), enable_amnezia = true;
    else if (query.hasQueryItem("headerprotectionkey"))
        header_protection_key = QueryValue(query, "headerprotectionkey"), enable_amnezia = true;
    if (query.hasQueryItem("content_padding_addition"))
        content_padding_addition = QueryValue(query, "content_padding_addition"), enable_amnezia = true;
    else if (query.hasQueryItem("contentpaddingaddition"))
        content_padding_addition = QueryValue(query, "contentpaddingaddition"), enable_amnezia = true;
