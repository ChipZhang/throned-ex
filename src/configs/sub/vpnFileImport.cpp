...
vpnfiRemote vpnfiParseRemote(const QStringList& args) {
    ...
    for (int i = 2; i < args.size() && i < 4; i++) {
        bool numeric = false;
        if (const auto port = args[i].toInt(&numeric); numeric && port > 0)
            remote.port = port;
        else if (auto network = vpnfiNetwork(args[i]); !network.isEmpty())
...
bool ParseOvpnConfig(const QString& body, openvpn& out, QStringList* problems) {
    ...
    for (const auto& item: directives) {
        ...
        if (!item.inlineTag.isEmpty()) {
            ...
            if (tag == "connection") {
                ...
                for (const auto& line: item.inlineBody) {
                    ...
                    if (args[0] == "remote")
                        remote = vpnfiParseRemote(args);
                    else if (args[0] == "proto" && args.size() > 1)
                        blockNetwork = vpnfiNetwork(args[1]);
                    else if ((args[0] == "port" || args[0] == "rport") && args.size() > 1)
                        blockPort = args[1].toInt();
                }
            ...
            if (tag == "ca")
                out.tls->certificate = item.inlineBody;
            else if (tag == "cert")
                out.tls->client_certificate = item.inlineBody;
            else if (tag == "key")
                out.tls->client_key = item.inlineBody;
                ...
                for (const auto& line: item.inlineBody) {
                    if (auto hex = vpnfiFingerprint(line); !hex.isEmpty())
                        out.tls->peer_fingerprint += hex;
                    else
                        notes << QObject::tr("Ignored an unreadable peer fingerprint: %1").arg(line);
                }
            } else if (tag == "pkcs12") {
        ...
        if (key == "proto") {
            if (auto network = vpnfiNetwork(value); !network.isEmpty())
                out.network = network;
            else if (value.endsWith("-server"))
                fatal = QObject::tr("`proto %1` is a server transport.").arg(value);
            else
                notes << QObject::tr("Unknown transport: proto %1").arg(value);
            continue;
        }
        ...
        if (key == "ifconfig-ipv6" && args.size() > 1) {
            if (auto prefix = vpnfiNormalizePrefix(args[1]); !prefix.isEmpty())
                ...
                notes << QObject::tr("Ignored an unreadable IPv6 interface address: %1").arg(args[1]);
        ...
        if (key == "setenv") {
            ...
            if (value == "CLIENT_CERT" && setting == "0")
                clientCertSwitch = "setenv CLIENT_CERT 0";
            else if (value == "FRIENDLY_NAME" && !setting.isEmpty())
        ...
        if (key == "key-direction") {
            keyDirection = value == "1" ? "client" : value == "0" ? "server"
                                                                  : QString();
            continue;
        }
        if (key == "secret") {
            ...
            if (args.size() > 2) keyDirection = args[2] == "1" ? "client" : args[2] == "0" ? "server"
                                                                                           : QString();
            continue;
        }
        ...
        if (key == "ca" || key == "cert" || key == "key" || key == "crl-verify") {
            ...
            if (key == "ca")
                out.tls->certificate_path = value;
            else if (key == "cert")
        ...
        if (key == "tls-auth" || key == "tls-crypt" || key == "tls-crypt-v2") {
            ...
            if (key == "tls-auth" && args.size() > 2) {
                keyDirection = args[2] == "1" ? "client" : args[2] == "0" ? "server"
                                                                          : QString();
            }
            continue;
        }
        if (key == "peer-fingerprint" || key == "verify-hash") {
            if (auto hex = vpnfiFingerprint(value); !hex.isEmpty())
                out.tls->peer_fingerprint += hex;
            else
                notes << QObject::tr("Ignored an unreadable peer fingerprint: %1").arg(value);
            continue;
        }
        ...
        if (key == "compress") {
            ...
            if (mode == "lzo")
                out.compression_lzo = "yes";
            else if (mode.isEmpty())
                out.compression = "stub";
        ...
        if (key == "ping-restart") {
            if (value == "0")
                out.ping_restart_disabled = true;
            else
                out.ping_restart = vpnfiDuration(value);
            continue;
        }
        if (key == "reneg-sec") {
            if (value == "0")
                out.renegotiate_disabled = true;
            else
                out.renegotiate_interval = vpnfiDuration(value);
            continue;
        }
        if (key == "reneg-bytes") {
    ...
    if (!clientCertSwitch.isEmpty()) {
        ...
        if (dropped) {
            notes << QObject::tr(
                         "`%1` turns the client certificate off; it was dropped and the server has to accept "
                         "password login.")
                         .arg(clientCertSwitch);
        }
    }
    ...
    if (!ifconfigLocal.isEmpty() && !ifconfigSecond.isEmpty()) {
        ...
        if (parsed.second >= 0)
            out.address += ifconfigLocal + "/" + QString::number(parsed.second);
        else
            notes << QObject::tr("Ignored an unreadable interface address: %1").arg(ifconfigLocal);
    ...
    if (!keyDirection.isEmpty()) {
        if (out.tls->control_wrap->type == "tls_auth")
            out.tls->control_wrap->direction = keyDirection;
        else if (out.mode == "static_key")
            out.key_direction = keyDirection;
    }
    // Outside static-key mode `cipher` is the pre-negotiation data cipher.
    if (!legacyCipher.isEmpty()) {
        if (out.mode == "static_key")
            out.cipher = legacyCipher;
        else if (out.data_ciphers_fallback.isEmpty())
            out.data_ciphers_fallback = legacyCipher;
    }
...
void vpnfiApplyOcOption(openconnect& out, const QString& name, const QString& value, bool hasValue, QStringList& notes) {
    if (name == "protocol") {
        ...
        if (const auto flavor = value.toLower(); flavors.contains(flavor))
            out.flavor = flavor;
        else
            notes << QObject::tr("Unsupported OpenConnect protocol, ignored: %1").arg(value);
        return;
    }
    if (name == "juniper") {
        out.flavor = "nc";
    ...
    if (name == "compression") {
        if (const auto mode = value.toLower(); mode == "none")
            out.compression_disabled = true;
        else if (mode == "stateless" || mode == "all")
            out.compression_mode = mode;
        else
            notes << QObject::tr("Unknown compression mode, ignored: %1").arg(value);
        return;
    }
    ...
    if (name == "os") {
        ...
        if (reported.contains(value.toLower()))
            out.reported_os = value.toLower();
        else
            notes << QObject::tr("Unknown reported OS, ignored: %1").arg(value);
        return;
    }
    if (name == "csd-wrapper") {
    ...
    if (name == "token-mode") {
        ...
        const auto mode = value.toLower();
        if (mode == "rsa" || mode == "stoken")
            out.token->mode = "stoken";
        else if (mode == "totp" || mode == "hotp" || mode == "oidc")
            out.token->mode = mode;
        else
            notes << QObject::tr("Unsupported token mode, ignored: %1").arg(value);
        return;
    }
    if (name == "token-secret") {
        if (value.startsWith('@'))
            out.token->secret_path = value.mid(1);
        else
            out.token->secret = value;
        return;
    }
    if (name == "form-entry") {
...
bool vpnfiReadAnyConnectHosts(const QString& xml, QList<vpnfiHostEntry>& out, QStringList& notes) {
    ...
    while (!reader.atEnd()) {
        ...
        if (token == QXmlStreamReader::StartElement) {
            ...
            if (element.compare("HostName", Qt::CaseInsensitive) == 0)
                current.name = text;
            else if (element.compare("HostAddress", Qt::CaseInsensitive) == 0)
                current.address = text;
            else if (element.compare("UserGroup", Qt::CaseInsensitive) == 0)
                current.group = text;
            else if (element.compare("PrimaryProtocol", Qt::CaseInsensitive) == 0)
                current.protocol = text;
            else if (element.endsWith("CertificateHash", Qt::CaseInsensitive))
                current.fingerprints += text;
            continue;
        }
    ...
    if (vpnfiIsOcCommandLine(body)) {
        ...
        for (const auto& line: body.split('\n')) {
            ...
            if (tokens[0].contains('='))
                ...
                argv += "--" + tokens[0] + "=" + tokens.mid(1).join(' ');
            // A lone word that names no option is the bare server address.
                     ...
                     vpnfiOcIgnored.contains(tokens[0]))
                argv += "--" + tokens[0];
            else
                argv += tokens[0];
        }
