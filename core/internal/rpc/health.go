...
func (s *server) Health(ctx context.Context, in *gen.HealthRequest) (*gen.HealthResponse, error) {
	...
	var core probe.CoreResolver
	if router := service.FromContext[adapter.DNSRouter](box.Context()); router != nil {
		core = coreLookup{router: router}
	}

	def := outbounds.Default()
	var info probe.IPInfo
	var clock probe.ClockReading
	var externalErr error
	var udp *probe.UDPTestResult
	var comparison probe.DNSComparison
	...
	if def != nil {
		...
		go func() {
			defer probes.Done()
			client, closeClient := probe.OutboundHTTPClient(ctx, def, healthTimeout)
			defer closeClient()
			info, clock, externalErr = probe.ExternalAddress(ctx, client)
		}()
		go func() {
			defer probes.Done()
			// A DNS round trip over UDP: the cheapest honest answer to "will calls work".
			udp = probe.ProbeUDP(ctx, def, "", 3, 0)
		}()
	}
	probes.Add(1)
	go func() {
		defer probes.Done()
		comparison = probe.CompareDNS(ctx, core, probe.DefaultSystemResolver(), domain)
	}()
	probes.Wait()
